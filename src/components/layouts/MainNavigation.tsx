...
function AccountMenu({ name, email, onSignOut }: { name: string; email?: string; onSignOut: () => void }) {
    ...
    const close = React.useCallback(() => { if (detailsRef.current) detailsRef.current.open = false; }, []);
    React.useEffect(() => {
    ...
    return <details ref={detailsRef} className="group relative"><summary className="flex h-10 cursor-pointer list-none items-center gap-2 rounded-full px-3 text-sm font-medium transition hover:bg-slate-100 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-slate-400 [&::-webkit-details-marker]:hidden"><span className="grid size-7 place-items-center rounded-full bg-violet-50 text-violet-400"><UserRound className="size-4" /></span><span className="hidden max-w-28 truncate xl:inline">{name.split(" ")[0]}</span><ChevronDown className="size-3.5 text-slate-400 transition-transform group-open:rotate-180" /></summary><div className="absolute right-0 top-[calc(100%+.65rem)] z-50 w-64 overflow-hidden rounded-2xl border border-slate-200 bg-white p-2 shadow-[0_24px_65px_-25px_rgba(15,23,42,.4)]"><div className="border-b border-slate-100 px-3 pb-3 pt-2"><p className="truncate text-sm font-semibold text-slate-900">{name}</p>{email && <p className="mt-0.5 truncate text-[11px] text-slate-400">{email}</p>}</div><nav className="py-2"><AccountMenuLink to="/profile" icon={<UserRound />} label="My profile" onSelect={close} /><AccountMenuLink to="/orders" icon={<Package />} label="My orders" onSelect={close} /><AccountMenuLink to="/wishlist" icon={<Heart />} label="Wishlist" onSelect={close} /></nav><button type="button" onClick={() => { close(); onSignOut(); }} className="flex w-full items-center gap-3 rounded-xl border-t border-slate-100 px-3 py-3 text-left text-sm font-medium text-violet-500 transition hover:bg-violet-50"><LogOut className="size-4" />Sign out</button></div></details>;
}

function AccountMenuLink({ to, icon, label, onSelect }: { to: string; icon: React.ReactNode; label: string; onSelect: () => void }) {
    return <Link to={to} onClick={onSelect} className="flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium text-slate-600 transition hover:bg-slate-50 hover:text-slate-950"><span className="text-slate-400 [&>svg]:size-4">{icon}</span>{label}</Link>;
}
