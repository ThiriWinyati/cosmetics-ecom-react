...
function AdminUserMenu({ name, email, onSignOut }: { name: string; email?: string; onSignOut: () => void }) {
  return <details className="group relative"><summary className="flex h-11 cursor-pointer list-none items-center gap-2 rounded-xl border border-slate-200 bg-white px-2 transition hover:bg-slate-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-slate-300 dark:border-white/10 dark:bg-white/5 dark:hover:bg-white/8 [&::-webkit-details-marker]:hidden"><span className="grid size-8 shrink-0 place-items-center rounded-lg bg-violet-50 text-xs font-bold text-violet-600 dark:bg-violet-900/50 dark:text-violet-200">{name.slice(0, 2).toUpperCase()}</span><span className="hidden max-w-24 truncate text-xs font-semibold xl:block">{name}</span><ChevronDown className="size-3.5 text-slate-400 transition group-open:rotate-180" /></summary><div className="absolute right-0 top-[calc(100%+.65rem)] z-60 w-64 overflow-hidden rounded-2xl border border-slate-200 bg-white p-2 shadow-[0_25px_70px_-25px_rgba(15,23,42,.45)] dark:border-white/10 dark:bg-[#1d1924]"><div className="px-3 py-3"><p className="truncate text-sm font-semibold">{name}</p><p className="mt-1 truncate text-[11px] text-slate-400">{email ?? "Store administrator"}</p><span className="mt-2 inline-flex rounded-full bg-emerald-50 px-2 py-1 text-[9px] font-semibold uppercase tracking-wider text-emerald-700">Administrator</span></div><div className="border-t border-slate-100 py-2 dark:border-white/8"><Link to="/" className="flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm text-slate-600 transition hover:bg-slate-50 dark:text-stone-300 dark:hover:bg-white/8"><ExternalLink className="size-4 text-slate-400" />View storefront</Link><Link to="/admin/chat" className="flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm text-slate-600 transition hover:bg-slate-50 dark:text-stone-300 dark:hover:bg-white/8"><MessageCircle className="size-4 text-slate-400" />Support inbox</Link></div><button type="button" onClick={onSignOut} className="flex w-full items-center gap-3 border-t border-slate-100 px-3 py-3 text-left text-sm font-medium text-violet-500 transition hover:bg-violet-50 dark:border-white/8 dark:text-violet-200 dark:hover:bg-violet-900/30"><LogOut className="size-4" />Sign out</button></div></details>;
}
